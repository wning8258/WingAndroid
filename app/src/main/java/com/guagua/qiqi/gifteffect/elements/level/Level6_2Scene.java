...
public class Level6_2Scene extends IScene implements BitmapOnDrawListener {
	...
	public Level6_2Scene(Context context, int number, int width, int height) {
	...
	public Level6_2Scene(Context context, int width, int height) {
...
//
		...
		if (bitmap != null) {
			for (int i = 0; i < 6; i++) {
				bitmapShape = new BitmapShape(bitmap, this);
				ElfFactory.endowLevelStar(bitmapShape, measureRect.left + PXUtils.dp2px(mContext, 16 + i * 10), measureRect.top - PXUtils.dp2px(mContext, 3),
						mContext);
				infoPanel.addInnerElement(bitmapShape);
			}
		}  
